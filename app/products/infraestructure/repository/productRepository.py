from typing import List
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy.future import select
from app.common.infraestructure.models import Producto
from app.products.domain.aggregate.product_aggregate import ProductAggregate
from app.products.domain.ports.IProductRepository import IProductRepository
from app.products.infraestructure.mappers.aggregate_to_model import aggregate_to_model
from app.products.infraestructure.mappers.model_to_domain import model_to_domain

class ProductRepository(IProductRepository[ProductAggregate]):
    def __init__(self, session: AsyncSession):
        self.session = session

    async def create_product(self, product_aggregate: ProductAggregate) -> None:
        product_model = aggregate_to_model(product_aggregate)
        self.session.add(product_model)
        await self.session.commit()
        await self.session.refresh(product_model)

    async def update_product(self, product_aggregate: ProductAggregate) -> ProductAggregate:
        product_model = aggregate_to_model(product_aggregate)
        await self.session.merge(product_model)
        await self.session.commit()
        return product_aggregate
    
    async def delete_product(self, product_id: str) -> None:
        result = await self.session.execute(select(Producto).where(Producto.id == product_id))
        product_model = result.scalar_one_or_none()
        if product_model:
            await self.session.delete(product_model)
            await self.session.commit()

    async def get_product_by_id(self, product_id: str) -> ProductAggregate:
        result = await self.session.execute(select(Producto).where(Producto.id == product_id))
        product_model = result.scalar_one_or_none()
        if product_model:
            return model_to_domain(product_model)
        return None
    
    async def get_product_by_code(self, product_code: str) -> ProductAggregate:
        result = await self.session.execute(select(Producto).where(Producto.codigo == product_code))
        product_model = result.scalar_one_or_none()
        if product_model:
            return model_to_domain(product_model)
        return None
        
    async def get_products(self) -> List[ProductAggregate]:
        result = await self.session.execute(select(Producto))
        product_models = result.scalars().all()
        return [model_to_domain(product_model) for product_model in product_models]